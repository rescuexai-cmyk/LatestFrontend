...
/// from the top edge so black letterboxing in `plan_ride_sheet_background.png`
/// is not visible — without bottom-reanchoring the mandala.
Widget planRideSheetBackgroundAsset(
  String asset, {
  double topCropPx = 24,
  Color errorColor = Colors.white,
  FilterQuality filterQuality = FilterQuality.medium,
  bool applyLaterGrayTone = false,
}) {
  Widget img = Image.asset(
    ...
    errorBuilder: (_, __, ___) => ColoredBox(color: errorColor),
  );
  if (applyLaterGrayTone) {
    img = Opacity(
      opacity: ScheduleLaterSheetAppearance.grayscaleMandalaOpacity,
      child: ColorFiltered(
        colorFilter: ScheduleLaterSheetAppearance.grayscaleMandalaFilter,
        child: img,
      ),
    );
  }
...
class _FindTripScreenState extends ConsumerState<FindTripScreen> {
  ...
  /// User tapped inner "Later" on ride-now chrome — show gray scheduling UI without route flags.
  bool _explicitLaterScheduleMode = false;
      ...
      body: Stack(
        children: [
          ...
          DraggableScrollableSheet(
            ...
            // Default ~55% viewport so ride list + slide control are visible without dragging.
            initialChildSize: 0.55,
            minChildSize: 0.28,
            maxChildSize: 0.80,
            snap: true,
            snapSizes: const [0.28, 0.55, 0.72, 0.80],
  ...
  Widget _buildBottomSheet(ScrollController scrollController) {
    ...
    final radius = BorderRadius.only(
      ...
      topRight: Radius.circular(figmaRideSheetTopRadius),
    );
          ...
          children: [
            ...
            if (_usesLaterBookingChrome) ...[
              const DecoratedBox(
                decoration: BoxDecoration(
                  gradient:
                      ScheduleLaterSheetAppearance.backgroundGradient,
                ),
              ),
              Positioned.fill(
                child: planRideSheetBackgroundAsset(
                  ScheduleLaterSheetAppearance.grayMandalaAsset,
                  topCropPx: 28,
                  errorColor: Colors.white,
                  filterQuality: FilterQuality.high,
                  applyLaterGrayTone: true,
                ),
              ),
            ] else ...[
              DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    end: const Alignment(0.05, 0.38),
                    colors: [
                      const Color(0x33CF923D),
              ...
              children: [
                Expanded(
                  ...
                  if (showTripCapsules) ...[
                    ...
                    // Surge / savings chips (compact, below handle)
                    if (_isSurgeActive)
                      Container(
                        padding: const EdgeInsets.symmetric(
...
class _PlanRideMultiStopRail extends StatelessWidget {
  ...
  final Color gold;
  final int stopCount;
  static const double _topRing = 20;
  static const double _bottomRing = 17;
  static const double _stopNode = 17;
  static const double _railWidth = 20;
  /// Nudge nodes to align with pick-up / drop-off text rows (Figma marks).
  static const double _pickupNudgeDown = 4;
  static const double _dropoffNudgeUp = 4;
  @override
  Widget build(BuildContext context) {
    final nodes = <Widget>[
      Transform.translate(
        offset: const Offset(0, _pickupNudgeDown),
        child: Container(
          width: _topRing,
          height: _topRing,
          decoration: BoxDecoration(
      ...
      Transform.translate(
        offset: const Offset(0, -_dropoffNudgeUp),
        child: SizedBox(
          width: _bottomRing,
          height: _bottomRing,
          child: Stack(
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          Positioned(
            left: (_railWidth - 2) / 2,
            top: _topRing + _pickupNudgeDown,
            bottom: _bottomRing + _dropoffNudgeUp,
...
/// Outline for Plan Your Ride chips — lighter than near-black for softer UI.
const Color _planRideChipBorder = Color(0xFFB5B2AC);

/// Figma: "Add Stop" pill — no fill, solid border only (matches updated design).
...
class _AddHomeSheet extends ConsumerStatefulWidget {
  ...
  /// When true, renders only inner content (no sheet chrome); parent should wrap in [Expanded].
  final bool embedded;

  /// Called instead of [Navigator.pop] when [embedded] is true. Same semantics as modal pop value.
  final void Function(bool saved)? onDismiss;
...
class _AddHomeSheetState extends ConsumerState<_AddHomeSheet> {
  ...
  void _popOrDismiss() {
    if (widget.embedded) {
      widget.onDismiss?.call(_savedThisSession);
    } else {
      Navigator.pop(context, _savedThisSession);
    }
  }
  ...
  double _successBannerBottomOffset(BuildContext context) {
    ...
    // Matches footer: SafeArea + `EdgeInsets.fromLTRB(16, 12, 16, 12)` + button block.
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.embedded) return layered;
        ...
        children: [
          Positioned(
            ...
            child: planRideSheetBackgroundAsset(
              _sheetBgAsset,
              topCropPx: 28,
              errorColor: const Color(0xFFFFF8F0),
              filterQuality: FilterQuality.high,
            ),
          ),
          layered,
...
class _LocationSearchSheet extends ConsumerStatefulWidget {
  ...
  const _LocationSearchSheet({
    ...
    this.useLaterBookingChrome = false,
...
class _LocationSearchSheetState extends ConsumerState<_LocationSearchSheet> {
  ...
  /// Plan Your Ride only: Add Home replaces sheet body in-place (no second modal).
  bool _inlineAddHome = false;
  ...
  Widget _buildPlanYourRideHeader() {
    ...
    final later = widget.useLaterBookingChrome;
    return Row(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        FigmaSquareBackButton(
        ...
        Expanded(
          child: Text(
            later ? 'Schedule Ride' : 'Plan Your Ride',
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
            style: GoogleFonts.poppins(
              fontSize: 16,
        ...
        child: Row(
          ...
          children: [
            _PlanRideMultiStopRail(
              gold: widget.useLaterBookingChrome ? Colors.black : _planOrange,
  ...
  Widget build(BuildContext context) {
    final bottomInset = MediaQuery.paddingOf(context).bottom;
    final showBookBar = _tripOverviewMode &&
        widget.onBookNowTap != null &&
        !_inlineAddHome;
    return Container(
      height: MediaQuery.of(context).size.height * 0.85,
      decoration: BoxDecoration(
        color: _tripOverviewMode && !widget.useLaterBookingChrome
            ? const Color(0xFFFFF8F0)
            : Colors.white,
        borderRadius: const BorderRadius.vertical(top: Radius.circular(32)),
        boxShadow: widget.useLaterBookingChrome
            ? ScheduleLaterSheetAppearance.modalTopShadow
            : [
                BoxShadow(
                  color: Colors.black.withValues(alpha: 0.06),
                  offset: const Offset(0, 4),
        ...
        children: [
          if (_tripOverviewMode)
            Positioned(
              ...
              right: -4,
              bottom: 0,
              child: widget.useLaterBookingChrome
                  ? Stack(
                      fit: StackFit.expand,
                      clipBehavior: Clip.hardEdge,
                      children: [
                        ...
                        const DecoratedBox(
                          decoration: BoxDecoration(
                            gradient: ScheduleLaterSheetAppearance
                                .backgroundGradient,
                          ),
                        ),
                        planRideSheetBackgroundAsset(
                          ScheduleLaterSheetAppearance.grayMandalaAsset,
                          topCropPx: 28,
                          errorColor: Colors.white,
                          filterQuality: FilterQuality.high,
          ...
          Column(
            children: [
          if (_tripOverviewMode && _inlineAddHome) ...[
            Center(
              child: Container(
                margin: const EdgeInsets.only(top: 12),
                width: 49,
            ...
            Expanded(
              child: _AddHomeSheet(
                ...
                onDismiss: (saved) {
                  if (!mounted) return;
                  setState(() => _inlineAddHome = false);
                  if (saved) _loadInitialSuggestions();
                },
              ),
            ),
          ] else ...[
